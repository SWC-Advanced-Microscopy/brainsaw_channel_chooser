...
2. The excitation chart was taller than the emission one. It is now slightly
   ...
   The first attempt only moved the problem: the emission chart grew into the
   space the excitation chart gave up. **Neither chart had ever been the height
   the stylesheet said.** Chart adds `sv-chart` to the container it is given, and
   `.sv-chart { height: 100% }` sat below `.chart-host` with equal specificity,
   so it won; the percentage resolved against a card of auto height — against the
   canvas the chart had just sized from the host — and settled wherever that loop
   settled, about 520 px. Only whichever host also carried an id rule was ever
   really pinned, which is why the two panels looked arbitrarily different and
   why the trouble jumped from one to the other when the id rule moved. The host
   is a plain block with a height; the chart inside it does not need one. Both
   are now the heights chosen for them, and **neither may exceed 70% of the
   viewport** however short the window gets.
